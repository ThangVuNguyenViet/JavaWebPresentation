...
public class InsertTicketController extends HttpServlet {
    ...
    private static final String INVALID = "insert_ticket.jsp";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (!priceChildStr.matches("\\d+000")) {
                valid = false;
                invalidObj.setPriceChildInvalid("Ticket Child Price must be a positive number and in thoundsand unit only");
            }
            if (!ticketLeftStr.matches("\\d+")) {
                valid = false;
                invalidObj.setTicketLeftInvalid("Ticket Left must be a positive number");
            }
            ...
            if (priceChildStr.isEmpty()) {
                valid = false;
                invalidObj.setPriceChildInvalid("Ticket Child Price cannot be empty");
            }
            if (ticketLeftStr.isEmpty()) {
                valid = false;
                invalidObj.setTicketLeftInvalid("Ticket Left cannot be empty");
            }
            ...
            if (valid) {
                ...
                TicketDTO dto = new TicketDTO(tourId, startDate, ticketLeft, priceAdult, priceChild);
                TicketDAO dao = new TicketDAO();
                boolean check = dao.insert(dto);
                if (check) {
                    url = SUCCESS;
                } else {
                    request.setAttribute("ERROR", "Insert Ticket Failed");
                }
            } else {
                url = INVALID;
